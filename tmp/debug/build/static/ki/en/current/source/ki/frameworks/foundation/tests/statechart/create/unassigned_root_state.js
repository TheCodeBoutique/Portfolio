...
module("Ki.Statechart: Create Statechart with Unassigned Root State Tests", {
  setup: function() {
    ...
    rootStateExample = Ki.State.extend({ test: 'foo' });
    
    obj3 = SC.Object.extend(Ki.StatechartManager, {
      owner: owner,
      initialState: 'stateE',
      rootStateExample: rootStateExample,
      stateE: Ki.State.design()
    });
    ...
    obj4 = SC.Object.extend(Ki.StatechartManager, {
      autoInitStatechart: NO,
      initialState: 'stateF',
      rootStateExample: rootStateExample,
      stateF: Ki.State.design()
    });
...
test("check obj2 statechart", function() {
  ...
  ok(SC.kindOf(rootState2, Ki.State), "root state should be kind of Ki.State");

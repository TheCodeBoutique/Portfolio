Thecodeboutique.apps = [
			// About Page //
		SC.Object.create({
			name: "Endeavour-os",
			show: "Thecodeboutique.appPage.mainPane.endeavourFrame",
			appImage:'/static/endeavourlight/en/current/resources/images/endeavour_icon.png?1299484324',
			description:"This is description about somthing",
			}),
		
				// About Page //
		SC.Object.create({
			name: "Hacktime",
			show: "Thecodeboutique.appPage.mainPane.hacktimeFrame",
			appImage:'/static/endeavourlight/en/current/resources/images/hacktime_icon.png?1299484324',
			description:"This is description about somthing",
			}),

			// Developers //
		SC.Object.create({
			name: "SC Community App",
			show: "Thecodeboutique.appPage.mainPane.comunityFrame",
			appImage:'/static/endeavourlight/en/current/resources/images/community_icon.png?1299484324',
			description:"This is description about somthing",
			}),
			// Demo Page //
		SC.Object.create({
			name: "Sproutcore Tutorials",
			show: "Thecodeboutique.appPage.mainPane.tutorialFrame",
			appImage:'/static/endeavourlight/en/current/resources/images/sproutcore_icon.png?1299484324',
			description:"This is description about somthing",
			}),
		SC.Object.create({
			name: "Animation",
			show: "Thecodeboutique.appPage.mainPane.animationFrame",
			appImage:'/static/endeavourlight/en/current/resources/images/animation.png?1299484324',
			description:"This is description about somthing",
			}),
			// Demo Page //
		SC.Object.create({
			name: "Game",
			show: "Thecodeboutique.appPage.mainPane.gameFrame",
			appImage:'/static/endeavourlight/en/current/resources/images/animation.png?1299484324',
			description:"This is description about somthing",
			})
];	; if ((typeof SC !== 'undefined') && SC && SC.Module && SC.Module.scriptDidLoad) SC.Module.scriptDidLoad('thecodeboutique');